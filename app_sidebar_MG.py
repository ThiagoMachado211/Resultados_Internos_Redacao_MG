...
FONT_SIZE = 24
MARKER_SIZE = 12
DEFAULT_XLSX = "data/Comparativo_MG.xlsx"  # ajuste se quiser outro arquivo padrão
# ==================================================
...
def grafico(base: pd.DataFrame, titulo: str):
    ...
    fig.update_layout(
        ...
        yaxis=dict(tickfont=dict(size=FONT_SIZE), title_font=dict(size=FONT_SIZE), range=[1.1*float(base["Nota"].min()),  0.9*float(base["Nota"].max())] ),
